...
int main(int argc, char **argv)
{
     ...
     */
    std::string license_path = "../../../id3Document.lic";
    /**
     * Fill in the correct path to the downloaded models.
     */
    std::string models_dir = "models/";
    /**
     * Fill in the correct path to the downloaded document template files.
     */
    std::string document_templates_dir = "document_templates/";
    /**
    ...
    err = id3DocumentLicense_CheckLicense(license_path.c_str(), nullptr);
     ...
     */
    std::cout << "Loading document detector 2B model" << std::endl;
    err = id3DocumentLibrary_LoadModel(models_dir.c_str(), id3DocumentModel_DocumentDetector2B, id3DocumentProcessingUnit_Cpu);
     ...
     */
    std::string template_path = document_templates_dir + "ALB_BO_01001_detector_2B_2.2.0.0.id3dr";
    std::cout << "Loading albanian ID template file" << std::endl;
    err = id3DocumentLibrary_LoadDocumentTemplate(template_path.c_str());
    check(err, "id3DocumentLibrary_LoadDocumentTemplate");
     ...
     */
    err = id3DocumentDetector_SetModel(detector, id3DocumentModel_DocumentDetector2B);
     ...
     *  ID3_DOCUMENT_RECTANGLE detection_zone{}
     */
	ID3_DOCUMENT_RECTANGLE detection_zone{};
	err = id3DocumentRectangle_Initialize(&detection_zone);
	check(err, "id3DetectedDocument_Initialize");
    err = id3DocumentRectangle_FromXywh(detection_zone, 0, 800, 2268, 2200);
    check(err, "id3DocumentRectangle_FromXywh");
	...
	ID3_DOCUMENT_STRING_LIST subset{};
	err = id3DocumentStringList_Initialize(&subset);
	check(err, "id3DocumentStringList_Initialize");
    err = id3DocumentDetector_DetectDocument(detector, image1, detection_zone, subset, detected_document);
     ...
     */
    int score;
    err = id3DetectedDocument_GetConfidence(detected_document, &score);
    check(err, "id3DetectedDocument_GetConfidence");
    ...
    else
    {
        ...
        check(err, "id3DocumentStringDict_Initialize");
         ...
         */
        ID3_DOCUMENT_STRING_LIST keys;
        err = id3DocumentStringList_Initialize(&keys);
        check(err, "id3DocumentStringList_Initialize");
        err = id3DocumentStringDict_GetKeys(field_dict, keys);
        check(err, "id3DocumentStringDict_GetKeys");

        int nb_keys;
        err = id3DocumentStringList_GetCount(keys, &nb_keys);
        check(err, "id3DocumentStringList_GetCount");

        for (int i = 0; i < nb_keys; i++)
        {
            int size = 44;
            char key[44];
            err = id3DocumentStringList_Get(keys, i, key, &size);
            check(err, "id3DocumentStringList_Get");
    ...
    err = id3DocumentLibrary_UnloadModel(id3DocumentModel_DocumentDetector2B, id3DocumentProcessingUnit_Cpu);
    err = id3DocumentLibrary_UnloadDocumentTemplate("alb_id");
}
